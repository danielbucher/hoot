# This module whas adapted from the code found in
# https://github.com/tenderlove/rails_autolink/blob/master/lib/rails_autolink/helpers.rb.
# Read the rails_autolink.LICENSE.md for a copy of the original software license.
...
module AutoLinkHelper
  ...
  def auto_link(text, *args, &block) #link = :all, html = {}, &block)
    ...
    when :all             then conditional_html_safe(auto_link_email_addresses(auto_link_urls(auto_link_hashtags(text, options[:html], options, &block), options[:html], options, &block), options[:html], &block), sanitize)
